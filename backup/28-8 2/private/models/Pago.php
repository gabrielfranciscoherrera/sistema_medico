...
class Pago {
    ...
    // Listar pagos recientes con datos útiles para el frontend
    public function listRecent($limit = 10) {
        $limit = max(1, (int)$limit);
        $sql = "SELECT 
                    pg.id as pago_id,
                    a.id_prestamo as prestamo_id,
                    pg.monto_pagado,
                    pg.fecha_pago,
                    e.nombre as cajero
                FROM pagos pg
                LEFT JOIN amortizaciones a ON pg.id_amortizacion = a.id
                LEFT JOIN empleados e ON pg.id_cajero = e.id
