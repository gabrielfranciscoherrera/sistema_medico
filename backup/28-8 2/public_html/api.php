...
switch ($action) {
    ...
    // --- Rutas de Préstamos ---
    case 'list_prestamos':
        // Cajero también necesita buscar préstamos para registrar pagos
        if (!has_permission(['Admin', 'Gerente', 'Servicio al Cliente', 'Cajero'])) deny_access();
    ...
    case 'update_prestamo_status':
        ...
        // Permitir a Cajero únicamente marcar como "Desembolsado".
        if (!is_authenticated()) deny_access('No autenticado.');
        $role = $_SESSION['user_role'] ?? null;
        $targetEstado = isset($data->estado) ? (string)$data->estado : '';

        $allowed = false;
        if (in_array($role, ['Admin', 'Gerente'])) {
            $allowed = true;
        } elseif ($role === 'Cajero' && $targetEstado === 'Desembolsado') {
            $allowed = true;
        }

        if (!$allowed) deny_access('No tiene permisos para cambiar el estado de este préstamo.');
        $prestamoController->updateStatus($data);
        break;
    case 'get_prestamo_pago_info':
        if (!has_permission(['Admin', 'Cajero', 'Gerente'])) deny_access('No tiene permisos para consultar pagos.');
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
        $prestamoController->getForPayment($id);
        break;
    ...
    case 'create_pago':
        ...
        break;
    case 'list_pagos_recientes':
        if (!has_permission(['Admin', 'Cajero', 'Gerente'])) deny_access('No tiene permisos para ver pagos.');
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $pagoController->listRecent($limit);
        break;
    ...
    case 'create_empleado':
        if (!has_permission(['Admin'])) deny_access();
        $empleadoController->create($data);
        break;
    case 'get_empleado':
        if (!has_permission(['Admin'])) deny_access();
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
